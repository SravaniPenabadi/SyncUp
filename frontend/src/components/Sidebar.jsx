...
const Sidebar = () => {
  const {
    ...
    isUsersLoading, deleteContact, getAllSyncScores, syncScores,
  } = useChatStore();
  const { onlineUsers } = useAuthStore();
  const [showOnlineOnly, setShowOnlineOnly] = useState(false);
  const [confirmDeleteId, setConfirmDeleteId] = useState(null);
  ...
  useEffect(() => {
    if (users.length > 0) {
      getAllSyncScores(users.map((u) => u._id));
    }
  }, [users]);
          ...
          <div key={user._id} className="relative group">
            ...
            >
              ...
              <div className="hidden lg:flex flex-1 items-center justify-between min-w-0">
                ...
                {/* ✅ Small sync meter badge */}
                {syncScores[user._id] && (
                  <SyncMeter
                    score={syncScores[user._id].syncScore}
                    label={syncScores[user._id].label}
                    size="sm"
                  />
                )}
              </div>
